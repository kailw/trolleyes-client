...
moduleTipousuario.controller('tipousuarioSelectionController', ['$scope', '$http', '$location', 'toolService',
    function ($scope, $http, $location, toolService) {
        ...
        $http({
            method: "GET",
            withCredential: true,
            url: "http://localhost:8081/trolleyes/json?ob=" + $scope.ob + "&op=getpage&rpp=" + $scope.numeroRegistrosPagina + "&page=" + $scope.numeroPagina
        ...
        $scope.seleccionar = function (id) {
            $http({
                method: "GET",
                withCredential: true,
                url: "http://localhost:8081/trolleyes/json?ob=" + $scope.ob + "&op=get&id="+ id
            }).then(function (response) {
                $scope.status = response.status;
